...
import type { CalendarSystem } from '../src/storage/preferences';

const OPTIONS: CalendarSystem[] = ['gregorian', 'persian', 'islamic'];
...
export default function CalendarPickerScreen() {
  ...
  function pick(value: CalendarSystem) {
    setPrefs({ calendar: value });
    router.back();
  }
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: colors.background }} contentContainerStyle={{ padding: spacing.md }}>
      <Section>
        {OPTIONS.map((value) => {
          const selected = prefs.calendar === value;
          return (
